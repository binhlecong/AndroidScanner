...
class ConstructorParamInspection :
    AndroidLintInspectionBase("Constructor Parameter Inspection", ConstructorParamIssue) {
...
class ConstructorParamIssueDetector : Detector(), Detector.UastScanner {
            ...
            if (rule[Config.FIELD_className].split('.').last() == constructor.name) {
                ...
                if (param.matches(reg)) {
                    if (rule[Config.FIELD_needFix].trim() != "1") {
                        context.report(
                            createConstructureParamIssue(rule, this),
                            node,
                        ...
                        context.report(
                            ...
                            context.getLocation(node),
                            rule[Config.FIELD_briefDescription],
                            createConstructorParamIssueQuickFix(context, node, rule, oldText)
                        )
                    }
                }
            }
    ...
    private fun createConstructorParamIssueQuickFix(
        context: JavaContext,
